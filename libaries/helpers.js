...
const randomString = function(string_length) {
...
const paginationLinks = function(path, currentUrl, total, page, limit) {
...
const moneyFormat = function(amount, sign) {
...
async function generateCode(
  orgId,
  module,
  { yearOfBirth, contactName, moduleName, productName }
) {
  const codeSetting = await AutoGeneratedCodeSetting.findOne({
    where: { orgId, moduleName: module }
  });
  const { setting } = codeSetting;
  if (!setting) {
    return "Settings not found.";
  }
  const { prefix, suffix, roots, separator } = setting.codeFormat;
  // console.log(prefix);
  // console.log(root);
  // console.log(suffix);
  ...
  configs.push({
    position: "prefix",
    type: prefix.type,
    typeFormat: prefix.typeFormat
  });
  for (const root of roots) {
    configs.push({
      position: "root",
      type: root.type,
      typeFormat: root.typeFormat
    });
  }
  configs.push({
    position: "suffix",
    type: suffix.type,
    typeFormat: suffix.typeFormat
  });
  // console.log(configs);
  const codes = [];
  for (const config of configs) {
    ...
    if (type === "issuingAuthority") {
      ...
      if (separator.type === "slash") {
        ...
        const dateInArrays = moment()
          .format("YYYY-MM-DD")
          .split("/");
        let dateString = "";
        for (const dateInArray of dateInArrays) {
